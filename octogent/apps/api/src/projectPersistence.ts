...
const GLOBAL_OCTOGENT_DIR = join(homedir(), ".octogent");
const PROJECTS_FILE = join(GLOBAL_OCTOGENT_DIR, "projects.json");
const PROJECT_CONFIG_RELATIVE_PATH = join(".octogent", "project.json");
...
type ProjectsRegistry = { projects: ProjectRegistryEntry[] };
...
const saveProjectsRegistry = (registry: ProjectsRegistry) => {
  ensureGlobalOctogentDir();
  writeFileSync(PROJECTS_FILE, `${JSON.stringify(registry, null, 2)}\n`, "utf8");
};

const resolveProjectConfigPath = (workspaceCwd: string) =>
...
const resolveGlobalProjectDir = (projectId: string) =>

...
import { OCTOPUS_COLORS, hashString, seededRandom } from "../../components/deck/octopusVisuals";
...
// Jitter determinístico: devuelve un offset (x,y) estable para un mismo
// nodeId, evitando que React Strict Mode (doble render) produzca saltos
// entre la primera y segunda pasada del render.
const sessionJitter = (seed: string): { x: number; y: number } => {
  const rng = seededRandom(hashString(seed));
  return { x: (rng() - 0.5) * 60, y: (rng() - 0.5) * 60 };
};
...
export const useCanvasGraphData = ({
  ...
  for (let i = 0; i < allTentacleIds.length; i++) {
    ...
    if (activeTerminals) {
      for (const activeTerminal of activeTerminals) {
        ...
        const jOffset = sessionJitter(sessionNodeId);

        const runtimeInfo = agentRuntimeStates?.get(activeTerminal.terminalId);
        const sessionNode: GraphNode = {
          id: sessionNodeId,
          type: "active-session",
          x: prevSession?.x ?? parentNode.x + jOffset.x,
          y: prevSession?.y ?? parentNode.y + jOffset.y,
          vx: prevSession?.vx ?? 0,
          vy: prevSession?.vy ?? 0,
  ...
  for (const terminal of columns) {
    ...
    const sessionNode: GraphNode = {
      id: sessionNodeId,
      type: "active-session",
      x: prevSession?.x ?? octobossNode.x + jOffset.x,
      y: prevSession?.y ?? octobossNode.y + jOffset.y,
      vx: prevSession?.vx ?? 0,
      vy: prevSession?.vy ?? 0,
      pinned: prevSession?.pinned ?? false,
      radius: ACTIVE_SESSION_RADIUS,
  ...
  for (const session of inactiveSessions) {
    ...
    const parentNode = currentNodesById.get(tentacleNodeId);
    ...
    const sessionNode: GraphNode = {
      ...
      x: prevSession?.x ?? parentX + jOffset.x,
      y: prevSession?.y ?? parentY + jOffset.y,
      vx: prevSession?.vx ?? 0,
      vy: prevSession?.vy ?? 0,
      pinned: prevSession?.pinned ?? false,
  ...
  // Actualizar el cache de posiciones después del commit. Mutar el ref
  // durante el render (como lo hacía la versión previa) viola las reglas
  // de React y, en Strict Mode, pisa el mapa con la primera pasada antes
  // de que la segunda lo consuma, perdiendo posiciones.
  useEffect(() => {

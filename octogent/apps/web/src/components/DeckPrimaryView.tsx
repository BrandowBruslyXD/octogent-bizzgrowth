...
import { useDeckTentacles } from "../app/hooks/useDeckTentacles";
import type { TerminalAgentProvider } from "../app/types";
import { buildDeckVaultFileUrl, buildTerminalsUrl } from "../runtime/runtimeEndpoints";
...
export const DeckPrimaryView = ({
  ...
  const {
    ...
    setCreateError,
    fetchTentacles,
    handleCreateTentacle: createTentacle,
    handleDeleteTentacle,
    handleDeleteAllTentacles,
    handleTentacleSkillsSave,
    handleTodoToggle,
  } = useDeckTentacles({ onRefreshWorkspaceSetup });
  ...
  const handleCreateTentacle = useCallback(
    async (
      ...
      const ok = await createTentacle(name, description, color, octopus, suggestedSkills);
      if (ok) setEmptyViewMode("idle");
    },
    [createTentacle],
  );
        ...
        <div className="deck-sidebar-content">
          ...
          {tentacles.length > 0 && (
            <div className="deck-sidebar-content-bottom">
              <DeckBottomActions onClearAll={handleDeleteAllTentacles} />
            </div>
          )}
        </div>
      ) : null,
